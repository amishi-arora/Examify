...
export default function RecentExams({ exams }) {
    return <div className="flex flex-col gap-4 w-full max-w-xl">
        ...
        {exams.map((e) =>
            <RecentExam key={e.examId} id={e.examId} title={e.title} questions={e.questions} difficulty={e.difficulty} date={e.date} results={e.results} studentAnswers={e.studentAnswers} insights={e.insights} />
        )}
    </div>
}